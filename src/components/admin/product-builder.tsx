...
export function ProductBuilder({ product }: ProductBuilderProps) {
  ...
  const [lessonDraft, setLessonDraft] = useState<Record<string, { title: string; videoUrl: string }>>({});
  ...
  async function createLesson(moduleId: string) {
    ...
    try {
      ...
      setLessonDraft((prev) => ({ ...prev, [moduleId]: { title: "", videoUrl: "" } }));
                  ...
                  <article key={module.id} className="rounded-xl border border-[var(--dourado)]/40 bg-white p-3">
                    ...
                    <div className="mt-3 grid gap-2 md:grid-cols-2">
                      <input
                        ...
                        onChange={(event) =>
                          setLessonDraft((prev) => ({ ...prev, [module.id]: { title: event.target.value, videoUrl: prev[module.id]?.videoUrl ?? "" } }))
                        }
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setLessonDraft((prev) => ({ ...prev, [module.id]: { title: prev[module.id]?.title ?? "", videoUrl: event.target.value } }))
                        }
                        className="rounded-lg border border-[var(--dourado)]/45 bg-white px-3 py-2 text-sm"
